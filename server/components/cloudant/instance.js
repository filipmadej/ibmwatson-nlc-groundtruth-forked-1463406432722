...
function updateView (dbconn, ddoc, existing, callback) {
    ...
    if (ddoc.views) {
        Object.keys(ddoc.views).forEach(function checkViewForUpdate (viewname) {
            var proposed = ddoc.views[viewname];
            var current = updated.views[viewname];
            if (isEligibleForUpdate(current, proposed)) {
                modified = true;
                updated.views[viewname] = proposed;
            }
        });
    }

    if (ddoc.updates) {
        var current = existing.updates;
        var proposed = ddoc.updates;
        if ( isEligibleForUpdate(current, proposed ) ) {
           modified = true;
           updated.updates = proposed;
        }
    }
